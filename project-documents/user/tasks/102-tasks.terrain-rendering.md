...
- [ ] **1.1 Create or switch to slice branch**
...
- [ ] **1.2 Add terrain config fields to `config.ts`**
...
- [ ] **2.2 Implement `parseTerrain` and extend `parseMessage` dispatch**
...
- [ ] **2.3 Unit tests for `parseTerrain`**
...
- [ ] **3.2 Add `applyTerrain` mutation helper**
...
- [ ] **3.3 Dispatch TERRAIN in `connection.handleMessage`**
...
- [ ] **3.4 Commit: state + connection dispatch for TERRAIN**
...
- [ ] **4.1 Add `getTerrainHeight` to `terrain.ts`**
...
- [ ] **4.2 Unit tests for `getTerrainHeight`**
...
- [ ] **5.2 Add `applyTerrainToMesh(mesh, grid)`**
  - In [src/rendering/terrain.ts](../../../src/rendering/terrain.ts), add and export:
    ```
...
- [ ] **5.3 Fallback path: `applyFlatPlane(mesh, worldWidth, worldHeight)`**
  - In [src/rendering/terrain.ts](../../../src/rendering/terrain.ts), add and export:
    ```
...
- [ ] **5.4 Unit-level sanity check: mesh vertex mapping and re-application**
...
- [ ] **6.2 Update entity tests for terrain-aware placement**
...
- [ ] **6.3 Commit: entity y-placement on terrain**
  - Commit message: `feat(entities): place instances on terrain surface`.

### 7. Main-loop integration

- [ ] **7.1 Detect terrain revision changes in the render loop**
...
- [ ] **7.2 Commit: main-loop terrain rebuild**
...
- [ ] **8.1 Pre-flight: build and unit tests green**
  - `pnpm tsc --noEmit` clean.
  - `pnpm test` all green (protocol, state, terrain, entities).
  - `pnpm build` succeeds.

- [ ] **8.2 Topography renders with a terrain-enabled server**
...
- [ ] **8.3 Perspective view shows topography**
...
- [ ] **8.4 Entities sit on the surface**
...
- [ ] **8.5 No-terrain fallback (unchanged behavior from slice 101)**
...
- [ ] **8.6 Second TERRAIN replaces the mesh (optional — run if server supports it)**
...
- [ ] **8.7 Malformed-frame tolerance (optional — run if easy)**
...
- [ ] **9.1 Update slice and task statuses**
...
- [ ] **9.2 Commit finalization**
  - Commit message: `docs: mark slice 102 complete`.

- [ ] **9.3 Merge and advance phase**
