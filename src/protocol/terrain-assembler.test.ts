...
function buildSnapshot(
  ...
  // 32-byte header + f64 payload (36 bytes/entity: 32 pos+vel + 4 profile idx).
  // schema_version=2 at offset 26; bytes 27-31 reserved (zero-filled).
...
function buildStateUpdate(
  ...
  const totalBytes = 16 + entityCount * 32;
  const buf = new ArrayBuffer(totalBytes);
  const view = new DataView(buf);
